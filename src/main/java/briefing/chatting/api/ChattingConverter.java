...
public static ChattingResponse.ChattingDetailResponseDTO toChattingDetailResponseDTO(
	Chatting chatting) {
	List<ChattingResponse.MessageResponseDTO> messageResponseDTOList =
		chatting.getMessages().stream().map(ChattingConverter::toMessageResponseDTO).toList();
...
public static ChattingResponse.ChattingListResponseDTO toChattingListResponseDTO(
	final List<Chatting> chattingList) {

	List<ChattingResponse.ChattingResponseDTO> chattingResponseDTOList =
		chattingList.stream().map(ChattingConverter::toChattingResponseDTO).toList();
	return ChattingResponse.ChattingListResponseDTO.builder()
		.chattings(chattingResponseDTOList)
		.build();
}

public static ChattingResponse.ChattingCreateResponseDTO toChattingCreateResponseDTO(
	Chatting chatting) {
...
public static List<ChattingRequest.MessageRequestDTO> getMessagesExcludeLast(
	List<ChattingRequest.MessageRequestDTO> messages) {
	return messages.subList(0, messages.size() - 1);
}

public static Optional<ChattingRequest.MessageRequestDTO> getLastMessage(
	List<ChattingRequest.MessageRequestDTO> messages) {
