...
public Message requestAnswer(final Long id, final ChattingRequest.AnswerRequestDTO request) {
	...
	final ChattingRequest.MessageRequestDTO lastMessage =
		ChattingConverter.getLastMessage(request.getMessages())
...
private void initChatting(
	...
	final List<Message> messages =
		ChattingConverter.getMessagesExcludeLast(request.getMessages()).stream()
			.map(
				message ->
					Message.builder()
						.chatting(chatting)
